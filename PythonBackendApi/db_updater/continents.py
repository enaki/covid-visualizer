...
def get_continents_ids(db_path):
    continents = {
        "North America": None,
        "Asia": None,
        "South America": None,
...
def insert_latest_continents(db_path, yesterday=False, two_days_ago=False, debug=True):
    ...
    for continent in latest_continents:
        continents_tuples.append((continent["continent"], date, continent["updated"], continent["cases"], continent["todayCases"], continent["deaths"],
    ...
    execute_many(latest_continents_sql_script, continents_tuples, database_path=db_path, message="latest continents", debug=debug)
