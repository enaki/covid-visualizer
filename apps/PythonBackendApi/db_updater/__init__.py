...
def update_latest_data(database_path, web_driver_folder_path, debug=True):
...
def init_database(database_path, drop_script_path, create_script_path, web_driver_folder_path):
    ...
    update_latest_data(database_path, web_driver_folder_path)
...
def schedule_updating_latest_data(database_path, web_driver_folder_path, interval_minutes=20):
    from datetime import datetime
    interval = 60 * interval_minutes
    # interval = 5
    while True:
        dt_string = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        update_latest_data(database_path, web_driver_folder_path, debug=False)
...
def initialize_database_workers(database_path, web_driver_folder_path):
    p1 = Process(target=schedule_updating_latest_data, args=(database_path, web_driver_folder_path, 20), daemon=True)
